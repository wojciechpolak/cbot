...
class ModifyCommand(Command):
    """
    `MODIFY` command: Modify task data.
    """

    async def execute(self, manager: TaskManager):
        task_id = int(self.operation.args[0])
        self.operation.output = manager.modify_task_data(task_id, self.operation)
