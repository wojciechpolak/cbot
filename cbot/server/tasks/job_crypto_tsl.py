...
class JobCryptoTsl(JobStrategy):
    async def run(self, task: Task):
        ...
        self.calc_above_offset_price(data)
        self.calc_stop_offset_price(data)
        ...
        async def on_task_modified(event: Dict):
            if event['taskId'] == task.id:
                ...
                self.calc_above_offset_price(data)
                self.calc_stop_offset_price(data, data.currentPrice)
        ...
        t1 = PeriodicTaskRunner(self.job_crypto_tsl_run, task=task)
    ...
    async def job_crypto_tsl_run(self, task: Task, exch: Exchange):
        printer = task.printer
        ...
        if data.algo == 'std2':
            is_done = self.calc_long_tsl_2(data, ticker)
        else:
            is_done = self.calc_long_tsl_1(data, ticker)
    ...
    def calc_above_offset_price(self, data: Data):
        if data.aboveInitialPriceOffsetPct:
            data.aboveInitialPriceOffset = data.aboveInitialPriceOffsetPct / Decimal('100') * data.initialPrice


    def calc_stop_offset_price(self, data: Data, price: Decimal = None):
        if price is None:
    ...
    def calc_long_tsl_1(self, data: Data, ticker) -> bool:
        try:
    ...
    def calc_long_tsl_2(self, data: Data, ticker) -> bool:
        try:
            data.currentPrice = Decimal(str(ticker.get('last')))
        except Exception:
            logger.exception('Exception: %s', ticker)
